...
clicked = ui.breadcrumb(breadcrumb_items, key="nav_breadcrumb")
...
When a breadcrumb item is clicked, the component returns a `BreadcrumbSelection` with:
- `text`: The text of the clicked item
- `href`: The href value (if provided)
- `index`: The index of the clicked item in the items list
